...
public class RegisterController {
	...
	public String doRegister(HttpServletRequest req, Model model, HttpSession session)
		 ...
		 * 
		 * @[a-zA-Z0-9.-]+：然後是@符號，後面可以包含字母、數字、點（.）或減號（-）。 \\.：然後是一個點（.），需要使用雙斜槓轉義。
...
//			verificationView.forward(req, res);
	...
	public String googleRegister(HttpServletRequest req, Model model) {
		...
		// ===資料有誤，回註冊頁面===
		if (!errMsgs.isEmpty()) {
			model.addAttribute("newMem", newMem);// 含有輸入格式錯誤的mem物件也存入
			model.addAttribute("errMsgs", errMsgs);
			return "login-google";
		} else {
			// ===資料正確，存入資料庫並導向會員中心===
			memSvc.addMem(newMem);
			session.setAttribute("mem", newMem); // 存進session
			session.setAttribute("google", true);
			model.addAttribute("mem", newMem);
