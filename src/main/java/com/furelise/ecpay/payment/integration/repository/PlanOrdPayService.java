...
@Service
public class PlanOrdPayService {

	@Autowired
	PlanOrdPayRepository dao;
	...
	public PlanOrdPay addPlanOrd(PlanOrdPayDTO req, Integer memID) {
		//狀態碼210003=待付款
		Integer planID = getPlanId(req.getPlanName(), req.getTimes());
		String day = getPickupDay(req.getWeekDay());

		PlanOrdPay planOrd = new PlanOrdPay(planID, req.getTimeID(), req.getPeriodID(), day, req.getWayID(), memID,
...
//			return planID;
	...
	public List<PlanOrdPay> getAllPlanOrd() {
		return dao.findAll();
	}

	public PlanOrdPay getPlanOrdById(Integer planOrdID) {
...
// ↓↓↓↓for show name instead of ID in list and detail PlanOrdController↓↓↓↓
	...
	public List<PlanOrdPayDTO> getPlanOrdInfo() {
		Integer planOrdId = 0;
		String memName = "";
		String planName = "";
		Date planStart = null;
		...
		List<PlanOrdPayDTO> infoList = new ArrayList<PlanOrdPayDTO>();

		List<PlanOrdPay> planOrdList = dao.findAll();
		for (PlanOrdPay p : planOrdList) {
			...
			PlanOrdPayDTO info = new PlanOrdPayDTO();
...
// ↑↑↑↑↑for list name instead of ID, PlanOrdController↑↑↑↑
	...
	public boolean verifyPlanOrdPurchase(Integer memID, String planStart) {
		...
		List<PlanOrdPay> planOrdList = dao.findByMemID(memID);
		// no purchase record
	...
	public java.util.List<PlanOrdPay> getAllMem(){
		return dao.findAll();
	}
